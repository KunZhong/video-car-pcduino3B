...
static void *ctl_thread(void *arg){
  ...
  http_listener(&server2,ctl_client_thread);
  return NULL;
}
...
static void signal_handler(int sigm) {
  ...
  pthread_join(server2.client, NULL);
  usleep(1000 * 1000);
  pthread_join(cd.tcam, NULL);
  pthread_join(cd.tctl, NULL);
  close_v4l2(cd.videoIn);
  free(cd.videoIn);
  if (close (server.sd) < 0) {
	  perror ("close sd");
  } 
  if (close (server2.sd) < 0) {
	  perror ("close sd");
  }
...
int main(int argc, char *argv[])
{
  ...
  while(1) {
    ...
    switch (option_index) {
      ...
      case 9:
	server2.port = htons(atoi(optarg));
        break;
      /* y */
      case 10:
        cd.format = V4L2_PIX_FMT_YUYV;
  ...
  pthread_create(&cd.tctl, NULL, ctl_thread, NULL);
  pthread_detach(cd.tctl);
  /*start http streamer */
  server.ptbuff = &tbuff;
  http_listener(&server,http_client_thread);
  return 0;
}
...
void help(char *progname)
{
  fprintf(stderr, "Usage: %s\n"
    ...
    " [-p ]          TCP-port for the stream server\n"
    " [-cp ]          TCP-ctl-port for the ctl server\n"
    " [-y ]                  use YUYV format\n"
